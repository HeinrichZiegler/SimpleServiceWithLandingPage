...
router.get('/', function (req, res, next) {
  res.render('index', { title: 'Simple Service',
                        contacts: contacts.list().result });
});
...
// any params are ignored, this is for view only
router.get('/contactsAll', (request, response, next) => {

    console.log('no params -- all');
    response.render('index', {title: 'Simple Service', 
                              contacts:contacts.list().result});
});
...
// example for using path variable, but for view only
router.get('/contact/lastname/:lastname', (request, response, next) => {
  ...
  const result = contacts.query_by_arg(
    "lastname", param);
  if (result) {
    response.render('index', {title: 'Simple Service - one result',
                              contacts: result}
                    );
  } else {
    next(createError(404));
